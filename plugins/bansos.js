...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 86400000) {
      if (Aku > Kamu) {
        conn.send2ButtonLoc(m.chat, await (await fetch(kbansos)).buffer(), 'Kamu Tertangkap Setelah Kamu korupsi dana bansos🕴️💰.  Dan kamu harus membayar denda 250 M 💵', watermark, 'Again', '.bansos', 'Dompet', '.wallet', m)
        user.money -= 250000000000
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 500000000000
        conn.send2ButtonLoc(m.chat, await (await fetch(mbansos)).buffer(), 'Kamu berhasil  korupsi dana bansos🕴️💰.  Dan kamu mendapatkan 500 M💵', watermark, 'Again', '.bansos', 'Dompet', '.wallet', m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else {
